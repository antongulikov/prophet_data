...
int get_word(string &str) {
    ...
    if (str.find("soil") == 0) return 0;
    if (str.find("alail") == 0) return 1;
    if (str.find("rte") == 0) return 2;
    if (str.find("arte") == 0) return 3;
    if (str.find("sitini") == 0) return 4;
    if (str.find("setini") == 0) return 5;
    return -1;
}
...
int main() {
    ...
    int mask = 0;
    while (std::cin >> str) {
        int group = get_word(str);
        if (group == -1) {
            std::cout << "NO" << std::endl; return 0;
        }
        mask |= (1 << (group & 1));
        vi.push_back(group / 2);
    }
    if (mask == 3) {
        std::cout << "NO" << std::endl; return 0;
    }
    ...
    if (count(vi.begin(), vi.end(), 1) != 1) {
        std::cout << "NO" << std::endl; return 0;
    }
    vi.erase(vi.begin(), unique(vi.begin(), vi.end()));
    if (count(vi.begin(), vi.end(), 0) > 1 ||
        count(vi.begin(), vi.end(), 2) > 1) {
            std::cout << "NO" << std::endl; return 0;
    }
    if (count(vi.begin(), vi.end(), 0) && vi[0] != 0) {
        std::cout << "NO" << std::endl; return 0;
    }
    if (count(vi.begin(), vi.end(), 2) && vi.back() != 2) {
        std::cout << "NO" << std::endl; return 0;   
    }
